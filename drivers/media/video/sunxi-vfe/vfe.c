...
static inline void vfe_set_addr(struct vfe_dev *dev,struct vfe_buffer *buffer)
{
  ...
  addr_org = videobuf_to_dma_contig((struct videobuf_buffer *)buf) - CPU_DRAM_PADDR_ORG;
...
//      printk("dev->mipi_fmt.packet_fmt[%d] = %x\n",i,dev->mipi_fmt.packet_fmt[i]);
  ...
  if(dev->is_isp_used) {
    ...
    ob_black_size.width= isp_size[MAIN_CH].width + win_cfg.hoffset; //OK
    ob_black_size.height= isp_size[MAIN_CH].height + win_cfg.voffset;//OK
  ...
  if(dev->is_isp_used && dev->is_bayer_raw) {
    ...
    switch (qc->id) {
      ...
      case V4L2_CID_AUTOGAIN:
        return v4l2_ctrl_query_fill(qc, 0, 1, 1, 1);
      case V4L2_CID_GAIN: 
        return v4l2_ctrl_query_fill(qc, 0, 64*16, 1, 0);
      case V4L2_CID_VFLIP:
