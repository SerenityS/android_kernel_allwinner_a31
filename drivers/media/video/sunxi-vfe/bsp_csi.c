...
int bsp_csi_set_base_addr(unsigned int sel, unsigned int addr)
{
...
int bsp_csi_set_size(unsigned int sel, struct bus_info *bus_info, struct frame_info *frame_info)
{
  ...
  for(i=0;i<row;i++) {
    ...
    for(j=0;j<column;j++) {
      ...
      csi_set_size(sel, ch, input_len_h[ch], input_len_v[ch], line_stride_y_row[i], line_stride_c_row[i]);
    }
  }
...
void bsp_csi_set_addr(unsigned int sel, unsigned int buf_base_addr)
{
  unsigned int buf_addr_plane0[MAX_CH_NUM];
  unsigned int buf_addr_plane1[MAX_CH_NUM];
  unsigned int buf_addr_plane2[MAX_CH_NUM];
...
void inline bsp_csi_int_get_status(unsigned int sel, unsigned int ch,struct csi_int_status *status)
{
  csi_int_get_status(sel, ch, status);
}

void inline bsp_csi_int_clear_status(unsigned int sel, unsigned int ch, enum csi_int_sel interrupt)
{
